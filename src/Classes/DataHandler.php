...
abstract class DataHandler{
	...
	private $_api_url = "http://api.thejournal.ie/v3/sample/";
	private $_username = "sample";
	private $_password = "eferw5wr335£65";
	private $_tag = "";
	...
	public function fetchAPI(): array{
		try{
			$curl_url = ($this->_tag) ? $this->_api_url . "tag/" . $this->_tag : $this->_api_url . "thejournal/";

			$process = curl_init($curl_url);
			curl_setopt($process, CURLOPT_USERPWD, $this->_username . ":" . $this->_password);
	...
	public function fetchFile(): array{
		...
		$file_location .= ($this->_tag) ? $this->_tag.".json" : "thejournal.json";
		try{
